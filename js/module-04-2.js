...
// function tags(countTags) {
//   countTags = tweets.map( (elem) => elem.tags.length);
...
// const workPractice = workers.reduce( (accum, worker) => accum + worker.years, 0 );
// console.log(workPractice);
...
// const filterArray = array.filter( (elem) => elem > 0 ).map( (elem) => Math.sqrt(elem) );
...
// const forengateToCelsius = celsius.map( (elem) => elem * 1.8 + 32);
// console.log(forengateToCelsius);
